...
class Solution:
  def __init__(self):
    ...
    self.q:deque[tuple[int,int]]=[]
  def bfs(self, grid: List[List[int]], mark: int)->int:
    cnt = 1
    while len(self.q)>0:
      v:tuple[int,int]=self.q.pop()
      for i in range(4):
        nextX, nextY = v[0] + self.dir[i][0], v[1] + self.dir[i][1]
        if nextX < 0 or nextX > len(grid) - 1 or nextY < 0 or nextY > len(grid[0]) - 1:
          continue
        if grid[nextX][nextY] == 0 or grid[nextX][nextY] > 1:
          continue
        self.q.append((nextX,nextY))
